  ...
  <div class="card-carousel">
    <div class="card-img">
      <div v-if="isView === true">
        ...
        <h4 class="subtitle" v-else style="text-align: center;">This image is was not approved with message
          "{{ messages[this.activeImage] }}".</h4>
      </div>
      <div v-else>
        <h2 class="title" style="text-align: center;">Entry from {{strings[this.activeImage]}}</h2>
      </div>

      <img class="large" :src="currentImage" alt="challenge photo">
      <div class="actions">
...
export default {
  name: 'Carousel',
  props: ['images', 'strings', 'approvals', 'messages', 'isView'],
  data() {
    return {
  ...
  computed: {
    ...
    currentImage() {
      this.$emit('update:data', this.activeImage);
